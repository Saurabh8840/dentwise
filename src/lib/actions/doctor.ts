...
export async function  getDoctors() {
    ...
    try {
        ...
        return doctors.map((doctors)=>({
            ...doctors,
            appointmentCount:doctors._count.appointments,
        }));
    } catch (error) {
...
export async function updateDoctor(input:updateDoctorInput){
    try {
        ...
        //if email is changing check if the email already exists
        ...
        const doctor=await prisma.doctor.update({
            ...
            //...input is goin to trigger the unique constraint violation for email
            data:{
                name:input.name,
                email:input.email,
                phone:input.phone,
